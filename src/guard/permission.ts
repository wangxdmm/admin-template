...
import { APP_INST } from '..'
import { useAuthStore } from ':/store/modules/auth'
import { useRouteStore } from ':/store/modules/route'
import { getToken } from ':/store/modules/auth/shared'
import type { RouteKey, RoutePath, StrategicPattern } from ':/types'
...
export function createPermissionGuard(router: Router) {
  router.beforeEach(async (to, from, next) => {
    ...
    const hasPermission
      = !routeRoles.length
      || authStore.userRoles.includes(SUPER_ADMIN)
      || authStore.userRoles.some((role: string) => routeRoles.includes(role))

    const strategicPatterns: StrategicPattern[] = [
...
async function createAuthRouteGuard(
  ...
  if (
    to.meta.constant
    && to.name !== APP_INST.config.global.home
    && !isNotFoundRoute
  ) {
    return true
  }
