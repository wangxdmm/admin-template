...
import { PageTab } from ':/materials/src'
...
const tabActiveSiblings = computed(() => {
  return getSiblingTab(tabStore.activeTabId)
})

const hoverSiblings = computed(() => {
  return getSiblingTab(hoverTab.value)
})

function getSiblingTab(activeFlag: string) {
  const siblings: {
    pre?: string
    next?: string
  } = {}

  tabStore.tabs.forEach((tab, index) => {
    if (tab.id === activeFlag) {
      const preIndex = index - 1
      const nextIndex = index + 1

      if (index > 0) {
        siblings.pre = tabStore.tabs[preIndex]?.id
        siblings.next = tabStore.tabs[nextIndex]?.id
      }
    }
  })
...
<template>
  <DarkModeContainer class="size-full flex-y-center">
      ...
      >
        ...
        >
          <PageTab
            ...
            :[TAB_DATA_ID]="tab.id"
            :hide-divider="
              tabActiveSiblings.pre === tab.id
                || hoverSiblings.pre === tab.id
