...
import { getColorByColorPaletteNumber, getColorPalette } from ':/color-palette'
import {
...
export function initThemeSettings(config: SystemConfig) {
  const themeSettings = merge({}, defaultThemeSettings)
...
export function createThemeToken(colors: ThemeColor) {
  ...
  const darkThemeTokens: ThemeToken = {
    colors: {
      ...themeTokens.colors,
 ...
 * @param tokens
 */
export function addThemeVarsToHtml(tokens: BaseToken, darkTokens: BaseToken) {
...
function getNaiveThemeColors(colors: ThemeColor) {
  const colorActions: NaiveColorAction[] = [
    ...
    {
      scene: 'Hover',
      handler: color => getColorByColorPaletteNumber(color, 400),
    },
    {
      scene: 'Pressed',
